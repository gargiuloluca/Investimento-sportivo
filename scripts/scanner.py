...
def fetch_csv(url):
    try:
        ...
        if resp.status_code == 200:
            # Fallback encoding per evitare crash su caratteri speciali
            text_data = resp.content.decode('latin-1')
            return pd.read_csv(io.StringIO(text_data))
    except Exception as e:
        print(f"Avviso: Impossibile scaricare {url}: {e}")
    return None
...
def build_league_profiles():
    ...
    for league_name, url in LEAGUE_HISTORICAL.items():
        ...
        for t in teams:
            ...
            h_cards = (h_games["HY"].sum() + h_games.get("HR", pd.Series(0, index=h_games.index)).sum() * 2) if has_cards else 0
            a_cards = (a_games["AY"].sum() + a_games.get("AR", pd.Series(0, index=a_games.index)).sum() * 2) if has_cards else 0
...
def main():
    print("Inizio elaborazione modelli statistici...")
    leagues_profile = build_league_profiles()
    fixtures_df = fetch_csv(FIXTURES_URL)
    
    upcoming_predictions = []
    
    # 1. Analisi fixtures reali se disponibili nel feed
    if fixtures_df is not None and not fixtures_df.empty:
        ...
        if all(c in fixtures_df.columns for c in valid_cols):
            for _, row in fixtures_df.iterrows():
                ...
                if profile:
                    pred = evaluate_match(league_name, h_team, a_team, d_str, t_str, profile)
                    if pred:
                        upcoming_predictions.append(pred)

    # 2. Se il feed fixtures è temporaneamente vuoto tra una giornata e l'altra,
    # analizza solo le sfide di vertice/prossimo turno senza duplicati incrociati
    if not upcoming_predictions:
        print("Palinsesto fixtures vuoto: calcolo accoppiamenti principali...")
        for league_name, profile in leagues_profile.items():
            teams = list(profile["teams"].keys())
            for i in range(0, len(teams) - 1, 2):
                h_t = teams[i]
                a_t = teams[i+1]
                pred = evaluate_match(league_name, h_t, a_t, "Prossimo Turno", "", profile)
                if pred:
                    upcoming_predictions.append(pred)
    ...
    print(f"Completato con successo! Generate {len(upcoming_predictions)} singole.")
